...
def heuristic(rts, cpus):
    ...
    def step8(cpu_stack, comm_stack, task_graph, communication_delay):
    ...
    for cpu_stack_perm_t in itertools.permutations(cpu_keys):
        ...
        # Perform an allocability test on all pre-allocated tasks
        for cpu, cpu_data in cpus.items():
            if not allocation_test(cpu_data["capacity"], cpu_data["tasks"]):
        ...
        #cpu_stack = [cpus[0], cpus[1], cpus[2], cpus[3], cpus[4], cpus[5]]
        ...
        if alloc_cnt > 0:
            ...
            valid_solution = step8(cpu_stack, comm_stack, task_graph, comm_delay)
    ...
    for idx, result in enumerate(results):
        ...
        for cpu_idx, cpu in enumerate(result):
            ...
            cpu_fu = sum(t["c"] / t["t"] for t in cpu)
            result_t.append((cpu_idx, ', '.join(str(t["id"]) for t in cpu), "{0} ({1:.2f}%)".format(cpu_mem, (cpu_mem / cpus[cpu_idx]["capacity"])*100), cpu_fu))
        print(tabulate(result_t, ["cpu", "tasks", "mem", "fu"], "psql"))
...
def main():
    ...
    # Increase by a factor the resource usage of each task
    for task in rts:
        task["r"] *= 1
    ...
    cpus0 = {0: {"capacity": 300, "tasks": []},
             ...
             4: {"capacity": 580, "tasks": []},
             5: {"capacity": 1060, "tasks": []}}
    ...
    cpus2 = {0: {"capacity": 16384, "tasks": []},
             1: {"capacity": 16384, "tasks": []},
             2: {"capacity": 16384, "tasks": []},
             3: {"capacity": 16384, "tasks": []}}
